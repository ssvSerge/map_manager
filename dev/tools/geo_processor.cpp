...
void geo_processor_t::geo_intersect ( const v_geo_coord_t& polyline, geo_rect_t& rect, const pos_type_t src, vv_geo_coord_t& clippedLine ) const {
    ...
    {   map_pos_t     dummy_pt1, dummy_pt2, dummy_pt3;
        ...
        v_geo_coord_t dummy_line;

        static int stop_cnt = 0;

        rect.min.get ( src, dummy_pt1 );
        rect.max.get ( src, dummy_pt2 );

        dummy.set_x  ( src, dummy_pt1.x );
        dummy.set_y  ( src, dummy_pt1.y );
        dummy_rect.push_back(dummy);

        dummy.set_x  ( src, dummy_pt2.x );
        dummy.set_y  ( src, dummy_pt1.y );
        dummy_rect.push_back(dummy);

        dummy.set_x  ( src, dummy_pt2.x );
        dummy.set_y  ( src, dummy_pt2.y );
        dummy_rect.push_back(dummy);
        ...
        for ( size_t i = 0; i < polyline.size(); i++ ) {
            polyline[i].get ( src, dummy_pt3 );
            dummy.set_x ( src, dummy_pt3.x );
            dummy.set_y ( src, dummy_pt3.y );
            dummy_line.push_back ( dummy );
        }
    ...
    _map_pt_pos ( polyline[0], rect, src, pos_prev );
    ...
    for ( size_t i = 1; i < polyline.size(); i++ ) {
        ...
        _map_pt_pos ( polyline[i], rect, src, pos_new );

        if ( pos_new == CODE_POS_IN ) {

            if ( pos_prev != CODE_POS_IN ) {
                if ( _get_intersection_pt ( polyline[i-1], polyline[i], pos_prev, rect, src, pt) ) {
        ...
        if ( pos_prev == CODE_POS_IN ) {
            ...
            if ( _get_intersection_pt ( polyline[i-1], polyline[i], pos_new, rect, src, pt) ) {
                segment.push_back(pt);
                if (segment.size() >= 2) {
