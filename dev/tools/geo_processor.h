...
class geo_processor_t {
    ...
    private:
        ...
        void _trim_idx_by_rect ( const geo_rect_t& base_rect, const l_geo_idx_rec_t& rect_list, v_geo_offset_t& out_list );
        void _idx_to_map ( const v_uint32_t& map_entries, l_geo_entry_t& map_cache );
        ...
        void _trim_record ( const geo_rect_t& rect_path, const geo_entry_t& geo_path, const bool is_area, geo_entry_t& out_record );
        void _process_area ( const geo_entry_t& geo_line, const bool force_clr, const bool mark_up );
        void _map_color ( const obj_type_t& obj_type, geo_pixel_t& border_color, geo_pixel_t& fill_color ) const;
        void _poly_area ( const v_geo_coord_t& region, const geo_pixel_t color );
        void _poly_line ( const v_geo_coord_t& line, const int width, const geo_pixel_t color );
        void _fill_poly ( const geo_rect_t& region, geo_rect_t& coords_list, const geo_pixel_t bk_clr, const geo_pixel_t fill_clr, const bool force_clr, const bool mark_up );
