...
#define VERTEX(e,p,s,x,y)  {add_vertex(&((e)->outp[(p)]->v[(s)]), x, y); (e)->outp[(p)]->active++;}

#define P_EDGE(d,e,p,i,j)  {(d)= (e); do {(d)= (d)->prev;} while (!(d)->outp[(p)]); (i)= (d)->bot.x + (d)->dx * ((j)-(d)->bot.y);}

#define N_EDGE(d,e,p,i,j)  {(d)= (e); do {(d)= (d)->next;} while (!(d)->outp[(p)]); (i)= (d)->bot.x + (d)->dx * ((j)-(d)->bot.y);}

#define MALLOC(p, b, s, t) {if ((b) > 0) { p= (t*)malloc(b); if (!(p)) { fprintf(stderr, "gpc malloc failure: %s\n", s); exit(0);}} else p= NULL;}
...
void gpc_read_polygon ( FILE* fp, int read_hole_flags, gpc_polygon* p ) {
    ...
    for (c = 0; c < p->num_contours; c++) {
        ...
        for (v = 0; v < p->contour[c].num_vertices; v++) {
            p->contour[c].vertex[v].x = 0;
            p->contour[c].vertex[v].y = 0;
            fscanf_s ( fp, "%lf %lf", &(p->contour[c].vertex[v].x), &(p->contour[c].vertex[v].y) );
        }
    }
}
