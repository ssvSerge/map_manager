...
static void _attach_objects ( void ) {
    ...
    std::cerr << "Preprocessing: " << added_cnt << "    \n";
    return;
}
...
static void _create_rects ( void ) {
    ...
    std::cerr << "Obj Rects: " << g_obj_rect_list.size() << "     \n";
}
...
static void _find_scale ( void ) {
    ...
    g_step_ver = scale * 250;
    g_step_hor = scale * 250;
}
...
static void _create_slicer_rects( void ) {
    ...
    for ( y = g_map_min.y; y <= g_map_max.y; y += g_step_ver ) {
        ...
        for ( x = g_map_min.x; x <= g_map_max.x; x += g_step_hor ) {
    ...
    std::cerr << "Map Rects [" << g_slicer_rects.size() << "][" << g_slicer_rects[0].size() << "]    \n";
}
