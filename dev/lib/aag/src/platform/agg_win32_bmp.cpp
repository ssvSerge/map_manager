...
namespace agg {
    ...
    pixel_map::pixel_map() : m_bmp(0), m_buf(0), m_bpp(0), m_is_internal(false), m_img_size(0), m_full_size(0) {
    }
    ...
    void pixel_map::create ( unsigned width, unsigned height, org_e org, unsigned clear_val ) {

        BITMAPINFO* bmpPtr = nullptr;
        ...
        bmpPtr = create_bitmap_info(width, height, m_bpp);
        create_from_bmp(bmpPtr);
    ...
    HBITMAP pixel_map::create_dib_section ( HDC h_dc, unsigned width, unsigned height, org_e org, unsigned clear_val ) {
    ...
    unsigned pixel_map::calc_full_size ( BITMAPINFO* bmp ) {
        ...
        return sizeof(BITMAPINFOHEADER) + sizeof(RGBQUAD) * calc_palette_size(bmp) + bmp->bmiHeader.biSizeImage;
    }
    ...
    BITMAPINFO* pixel_map::create_bitmap_info ( unsigned width, unsigned height, unsigned bits_per_pixel ) {
    ...
    void pixel_map::draw ( HDC h_dc, const RECT *device_rect, const RECT *bmp_rect ) const {
        ...
        if (dvc_width != bmp_width || dvc_height != bmp_height) {
            ...
            ::StretchDIBits( h_dc, dvc_x, dvc_y, dvc_width, dvc_height, bmp_x, bmp_y, bmp_width, bmp_height, m_buf, m_bmp, DIB_RGB_COLORS, SRCCOPY );
        } else {
            ::SetDIBitsToDevice ( h_dc, dvc_x, dvc_y, dvc_width, dvc_height, bmp_x, bmp_y, 0, bmp_height, m_buf, m_bmp, DIB_RGB_COLORS );
        }
    }
    ...
    void pixel_map::blend ( HDC h_dc, const RECT *device_rect, const RECT *bmp_rect ) const {
            ...
            void* buf = 0;
            HBITMAP bmp = ::CreateDIBSection( mem_dc, m_bmp, DIB_RGB_COLORS, &buf, 0, 0 );
            ...
            ::AlphaBlend ( h_dc, dvc_x, dvc_y, dvc_width, dvc_height, mem_dc, bmp_x, bmp_y, bmp_width, bmp_height, blend );
    ...
    HBITMAP pixel_map::create_dib_section_from_args ( HDC h_dc, unsigned width, unsigned height, unsigned bits_per_pixel ) {
